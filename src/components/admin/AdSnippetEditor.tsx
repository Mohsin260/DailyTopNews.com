...
export default function AdSnippetEditor({
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<TemplateKey | "">(() => {
    ...
    const media = editingPosition.mediaUrl || editingPosition.url || "";
    const isImageMedia =
      editingPosition.creativeType === "image" ||
      editingPosition.type === "image" ||
      /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(media);
    if (type === "direct_banner") return "image";
    if (type === "html_banner") return isImageMedia ? "image" : "html_banner";
    ...
    // Fallback: detect from existing code or structured media
    const code = editingPosition.code || "";
    if (code.includes('video-js') || code.includes('video/mp4')) return "video";
    if (code.includes('<img') || isImageMedia) return "image";
  ...
  // Seed/DB ads often store templateType + mediaUrl with empty code.
  // On open, inject the selected template's snippet so Custom Code shows
  // the image template with mediaUrl in src (not a blank textarea).
  const didAutoApplyRef = useRef(false);
  useEffect(() => {
    if (didAutoApplyRef.current) return;
    if (!selectedTemplate || selectedTemplate === "native_feed") return;
    if ((editingPosition.code || "").trim() !== "") return;
    if (!mediaUrl && selectedTemplate !== "html_banner") return;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ─── ENABLE TOGGLE (only when this position already has real ad content) ─── */}
      {(() => {
        const epNative = editingPosition.nativeContent;
        const epHasContent =
          editingPosition.templateType === "native_feed"
            ? !!(epNative?.title?.trim() || epNative?.image?.trim())
            : !!(
                editingPosition.code?.trim() ||
                editingPosition.mediaUrl?.trim() ||
                editingPosition.url?.trim() ||
                editingPosition.vastUrl?.trim() ||
                editingPosition.vastTagUrl?.trim()
              );
        if (!epHasContent) return null;
        return (
