...
export async function GET(req: Request) {
  try {
    ...
    const adsEnabled = await isAdsGloballyEnabled();
    // Public website queries use activeOnly — suppress them when global ads are off.
    // Dashboard (no activeOnly) still receives items so admins can manage ads.
    if (!adsEnabled && activeOnly) {
      return NextResponse.json(
        { items: [], adsEnabled: false },
    ...
    return NextResponse.json(
      { items: resolvedItems, adsEnabled },
      { headers: { "Cache-Control": "no-store, max-age=0" } }
    );
  } catch (err) {
