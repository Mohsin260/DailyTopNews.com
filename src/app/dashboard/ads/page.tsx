...
const isAdConfigured = (ad?: { code?: string; mediaUrl?: string; url?: string; vastUrl?: string; vastTagUrl?: string; nativeContent?: unknown; templateType?: string } | null): boolean => {
  if (!ad) return false;
  const native = ad.nativeContent as { title?: string; image?: string } | undefined;
  const nativeHasContent = !!(
    ((native?.title as string) || "").trim() ||
    ((native?.image as string) || "").trim()
  );
  if (ad.templateType === "native_feed") return nativeHasContent;
  return !!(
    ad.code?.trim() ||
    ad.mediaUrl?.trim() ||
    ad.url?.trim() ||
...
export default function AdsPage() {
    ...
    mutationFn: async (editData: EditingPosition) => {
      ...
      if (isEmpty) {
        ...
        const payload: Record<string, any> = {
          ...
          customCode: editData.customCode ?? false,
  ...
  const handleEditPosition = (pageType: PageType, position: string) => {
    ...
    setEditingPosition({
      ...
      customCode: (existingAd as any)?.customCode ?? false,
                    ...
                    {adsLoading ? (
                      ...
                      <div className="grid gap-4">
                        {config.positions.map((pos) => {
                          ...
                          const hasCode = isAdConfigured(ad);
